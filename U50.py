...
def findOverlaps(sorted_start, sorted_stop, length_array):
	...
	for i in xrange(0,len(sorted_start)):
		a = sorted_start[i]
		b = sorted_stop[i]
...
def contigCount(Unique_Count, Gap_Index):
	...
	for k,v in sorted(Unique_Count.items(), key=lambda (k,v): (v,k), reverse=True):
		sum = int(sum) + int(v)
		count = count + 1
...
def stats(median, Unique_Count):
	...
	for k,v in sorted(Unique_Count.iteritems(), key=lambda (k,v): (v,k), reverse=True):
		sum = int(sum) + int(v)
		count = count + 1
		if sum >= median:
			contig = k
...
with open(BED_file.split(".")[0]+"_contigs.txt", "w") as contig_outfile:
	...
	contig_outfile.writelines('{} : {}\n'.format(k,v) for k,v in sorted(Unique_Count.items(), key=lambda (k,v): (v,k), reverse=True))
